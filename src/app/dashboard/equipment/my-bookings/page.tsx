...
import { getMyEquipmentBookings, cancelEquipmentBooking } from "@/app/actions/equipment";
import Link from "next/link";
import { Calendar, Clock, Monitor, XCircle, ArrowLeft, Loader2, Info } from "lucide-react";
...
export default function MyEquipmentBookingsPage() {
  ...
  const handleConfirmCancel = async () => {
    ...
    const res = await cancelEquipmentBooking(selectedBookingId);
    if (res.success) {
      loadBookings();
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "APPROVED":
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">อนุมัติแล้ว</span>;
      case "REJECTED":
      ...
      case "RETURNED":
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">คืนแล้ว</span>;
      default:
  ...
  return (
    <>
      <ConfirmModal
        ...
        description="คุณแน่ใจหรือไม่ว่าต้องการยกเลิกการยืมอุปกรณ์นี้? การดำเนินการนี้ไม่สามารถเรียกคืนได้"
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center space-x-3 mb-1">
              <Link href="/dashboard/equipment" className="text-zinc-400 hover:text-zinc-900 transition-colors">
                <ArrowLeft className="w-5 h-5" />
              </Link>
              <h1 className="text-2xl font-bold text-zinc-900">ประวัติการยืมอุปกรณ์ของฉัน</h1>
            </div>
            <p className="text-zinc-500 ml-8">รายการยืมคืนอุปกรณ์ทั้งหมดของคุณ</p>
          </div>
          <Link
        ...
        {bookings.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-zinc-100 shadow-sm flex flex-col items-center">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 mb-1">ยังไม่มีประวัติการยืม</h3>
            <p className="text-zinc-500">คุณยังไม่ได้ทำการยืมอุปกรณ์ใดๆ ในระบบ</p>
          </div>
        ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-bold text-zinc-900 flex items-center">
                      <Monitor className="w-5 h-5 mr-2 text-zinc-400" />
